...
let myPara: string=answers.para ;
myPara=myPara.trim();
let myCharacters: string =myPara.replace(/\s/g,"");
let myWords: string []=myPara.split(" ")
let wordsCount=0;
...
function WordsCount( items:string[]) : number {
let n=0;
...
function CharactersCount( items:string[]) : number {
    let n=0;
        for (let a=0 ; a < items.length;a++)
        {
           items[a]=items[a].replace(/\s/g,""); 
